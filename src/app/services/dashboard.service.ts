...
export class DashboardService {
    ...
    constructor(private http: HttpClient, private initiateService: InitiateService) { }
    ...
    async updateCompanySettings(companyId: string): Promise<void> {
        ...
        try {
            ...
            await firstValueFrom(
                this.http.patch<any>(`${this.apiUrl}/settings/update-company`, { companyId }, { headers }).pipe(
                    tap((response) => {
                        if (response.token) {
                            // Save the new token
